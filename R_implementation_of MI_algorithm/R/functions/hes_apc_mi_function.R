...
# Version:      v0.1
#
# This script encapsulates the original '03_hes_apc_mi_function.R' script in a function.
# It identifies MI events in HES-APC based on the algorithm from
# the RIPCORD-2 trial (https://www.ahajournals.org/doi/10.1161/CIRCULATIONAHA.121.057793)

library(tidyverse)
library(lubridate)

# Function to process MI events in HES-APC
process_hes_apc_mi <- function(mi_hes_apc_input_path, output_mi_hes_apc_path) {
  
  # Read in the prepared HES-APC CIPS data
  hes_apc_prepared_cips = read_rds(mi_hes_apc_input_path)
  ...
  # Dataset of individuals with no MI diagnoses - assigned to terminal node 1
  ...
  hes_apc_persons_with_mi = hes_apc_selected %>% 
    ...
    group_modify(~ {
      ...
      # Loop over rows and apply decision tree algorithm
      for(i in 1:nrow(.x)){
        # Implement decision tree and MI event processing
            ...
            if (as.logical(.x[i, "episode_lt_28d_from_mi"])){
              ...
              if(as.logical(.x[i, "same_cips_as_last_mi"]) == FALSE){
                ...
                if(as.logical(.x[i, "any_gap_in_mi_diagnosis"]) == FALSE){
                  ...
                  # Yes - There is at least one episode between the previous MI
  ...
  write_rds(
    x = hes_apc_processed,
    file = file.path(output_mi_hes_apc_path, "hes_apc_mi_processed.rds")
  )
  
  write_csv(
    x = hes_apc_processed,
    file = file.path(output_mi_hes_apc_path, "hes_apc_mi_processed.csv")
  )
  ...
  write_rds(
    x = hes_apc_mi_events,
    file = file.path(output_mi_hes_apc_path, "hes_apc_mi_events.rds")
  )
  
  
  write_csv(
    x = hes_apc_mi_events,
    file = file.path(output_mi_hes_apc_path, "hes_apc_mi_events.csv")
  )
  
  
  message("MI processing for HES-APC data completed successfully.")
}