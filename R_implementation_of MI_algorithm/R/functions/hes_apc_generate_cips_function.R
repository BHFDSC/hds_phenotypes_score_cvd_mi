...
# 02_hes_apc_generate_cips_function.R
...
# Version:      v0.1
#
# This script encapsulates the original '02_hes_apc_generate_cips.R' script in a function.
# It generates continuous inpatient spell (CIPS) indicators based on 
...
generate_cips <- function(cips_input_path, output_cips_path) {
  # Parameters 
  ...
  # Load dataset
  hes_apc_prepared = read_rds(cips_input_path)
  ...
  write_csv(
    ...
    file = here::here(output_cips_path, "hes_apc_prepared_cips.csv")
  )
  
  # write to .rds file for importing to hes_apc_mi_function
  write_rds(
    x = hes_apc_prepared_cips,
    file = here::here(output_cips_path, "hes_apc_prepared_cips.rds")
  )
}
