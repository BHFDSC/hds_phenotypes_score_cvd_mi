...
# Version:      v0.1
#
# This script encapsulates the original '01_hes_apc_prepare.R' script in a function. 
...
prepare_hes_apc <- function(input_hes_apc_path, 
  ...
  write_csv(
    x = flowchart_hes_apc,
    file = file.path(output_hes_apc_path, "flowchart_hes_apc.csv")
  )
  ...
  write_csv(
    ...
    file = file.path(output_hes_apc_path, "hes_apc_excluded_rows.csv")
  )
  ...
  # write to .csv for portability 
  write_csv(
    x = hes_apc_prepared,
    file = file.path(output_hes_apc_path, "hes_apc_prepared.csv")
  )
  
  # write to .rds for reading back into hes_apc_cips_function.R script
  write_rds(
    x = hes_apc_prepared,
    file = file.path(output_hes_apc_path, "hes_apc_prepared.rds")
  )
