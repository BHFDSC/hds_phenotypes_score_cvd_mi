...
def temp_save(df, out_name:str, _dbc:str=f'', quietly=False):
...
# def snapshot(df, snapshot_name, snapshot_list_name, dbc=''):
#   """
#   test
...
def save_table(df, out_name:str, save_previous=True, data_base:str=f''):
  ...
  # assert isinstance(df, f.DataFrame), 'df must be of type dataframe' #isinstance(df, pd.DataFrame) |  # 20251009 - COMMENTED OUT FOLLOWING MIGRATION TO UNITY CATALOG
...
def table_summ(_data, _convarlist=[], _idvarlist=[], _sdc=False, _name=''):
  ...
  Status:
    In testing stage 1 - see /Workspaces//SHDS/Fionna/Function Testing/table_summ_test
...
def get_provisioning_date(year=None, month=None) -> None:
    ...
    if year is None and month is None:
        provisioning_date = (
        (spark.table(f'.wrang002b_data_version_batchids'))
...
def extract_batch_from_archive(_df_datasets: DataFrame, _dataset: str):
  ...
  _row = _df_datasets[_df_datasets['dataset'] == _dataset].toPandas()
