...
# _runtime_folder = os.environ.get('PROJECT_RUNTIME_FOLDER', None)
# assert _runtime_folder is not None, (
#     "PROJECT_RUNTIME_FOLDER not set. Ensure project_config has been run."
# )
...
def load_table(table: str, table_directory: str = None, method: str = None) -> DataFrame:
    ...
    #     table_directory = read_json_file(path=f'{_runtime_folder}/config/table_directory.json')
    ...
    if table_directory is None:
        import os
        _runtime = os.environ.get('PROJECT_RUNTIME_FOLDER')
        _path = f'{_runtime}/config/table_directory.json' if _runtime else './config/table_directory.json'
        table_directory = read_json_file(path=_path)
    elif isinstance(table_directory, str):
        table_directory = read_json_file(table_directory)
    else:
        raise ValueError("table_directory should be a string or None.")
